...
from interp.common.wrappers import CustomRewardSSWrapper, DummyWrapper
...
@ex.config
def config():
    env_id = "EmptyMaze-10x10-CoinFlipGoal-v3"        # the gym environment to use
    algo = "ppo"                         # the RL algorithm to use
    if 'Maze' not in env_id:
        raise Exception("Only maze environments allowed")
    timesteps = int(1e5)                 # number of timesteps to train the policy on
    device = get_device()                # the device to put/train the SB3 model on
    eval_freq = timesteps // 100        # frequency to evaluate a SB3 model and possibly save new best
    ...
    rm_env_id = env_id                  # which environment's regressed reward model to use, provided that regressed_reward=True
...
def run(
    ...
    with open(f'hyperparams/{algo}.yml', 'r') as f:
        ...
        hyperparams = hyperparams_dict['maze']
    ...
    def create_env(n_envs, eval_env=False, regressed_reward=False, no_log=False):
        ...
        if regressed_reward:
            rm_path = f"../reward-models/{rm_env_id}_ss-reward_model.pt"
